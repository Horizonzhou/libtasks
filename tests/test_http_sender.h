...
class test_compressed_handler : public tasks::net::http_response_handler {
  public:
    bool handle_response(std::shared_ptr<tasks::net::http_response> response);
};
...
class test_http_sender : public CppUnit::TestFixture {
    ...
    CPPUNIT_TEST(compressed_requests);
    CPPUNIT_TEST_SUITE_END();

    friend class test_handler;
    friend class test_compressed_handler;
  ...
  private:
    static bool m_compressed;
    static int m_status_code;
    static std::string m_content_type;
